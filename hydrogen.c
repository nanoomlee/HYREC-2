...
double rec_TLA_dxHIIdlna(double xe, double xHII, double nH, double H, double TM, double TR,
			 double Fudge, double fsR, double meR, double ion, double exclya) {
...
void interpolate_rates(double Alpha[2], double DAlpha[2], double Beta[2], double *R2p2s,
		       double TR, double TM_TR, HYREC_ATOMIC *atomic, double fsR, double meR, int *error, char error_message[SIZE_ErrorM]) {   
    double factor;
    unsigned l, k;
    long iTM, iTR;
    double frac1, frac2;
...
double rec_swift_hyrec_dxHIIdlna(double xe, double xHII, double nH, double H, double TM, double TR, HYREC_ATOMIC *atomic, FIT_FUNC *fit, double z, double fsR, double meR, double ion, double exclya, int *error, char error_message[SIZE_ErrorM], double wcb, double wb, double wnu, double Nur, double Yp, double mnu[3], int Nmnu, double pann){
   ...
   double X_2s, X_2p, C_2s, C_2p, gamma_2s, gamma_2p, s, Dxe2;
   ...
   if (Nmnu == 0) {
	   ...
	   diff[2] = Nur-(3.046-Nmnu);
   }

   interpolate_rates(Alpha, DAlpha, Beta, &R2p2s, TR, TM/TR, atomic, fsR, meR, error, error_message);
   alphaB = Alpha[0] + Alpha[1];
   four_betaB = Beta[0] + 3.*Beta[1];

   RLya = LYA_FACT(fsR, meR) *H/nH/(1.-xHII);   // 8 PI H/(3 nH x1s lambda_Lya^3) 
   DK_K_fid = rec_interp1d(fit->swift_func[0][0], 10., fit->swift_func[1], DKK_SIZE, TR/kBoltz, error, error_message);
   
   for (i = 0; i < 3; i++) {
	   DK_K_fid = DK_K_fid + rec_interp1d(fit->swift_func[0][0], 10., fit->swift_func[i+2], DKK_SIZE, TR/kBoltz, error, error_message)*diff[i];
   }
   DK_K = DK_K_fid;
   ...
   X_2s = nH*xe*xe*Alpha[0] - (1.-xe)*Beta[0]*exp(-E21/TR);
   X_2p = nH*xe*xe*Alpha[1] - (1.-xe)*3*Beta[1]*exp(-E21/TR);
   ...
   return -nH/H *( (s*(1.-xHII)*DAlpha[0] + Alpha[0]*Dxe2)*C_2s + (s*(1.-xHII)*DAlpha[1] + Alpha[1]*Dxe2)*C_2p)
           + (ion + (0.25*(1.-C_2s) + 0.75*(1.-C_2p))*exclya)/H ;
}
...
void solveTXeqB(double *diag, double *updiag, double *dndiag, 
                double *X, double *B, unsigned N, int *error, char error_message[SIZE_ErrorM]){
     int i;
     double denom;
...
double rec_HMLA_2photon_dxHIIdlna(double xe, double xHII, double nH, double H, double TM, double TR, 
   ...
   for (i = 0; i < 2; i++) Trv[i] = create_1D_array(NVIRT, error, error_message);
   for (i = 0; i < 2; i++) Tvr[i] = create_1D_array(NVIRT, error, error_message);
   for (i = 0; i < 3; i++) Tvv[i] = create_1D_array(NVIRT, error, error_message); 
   ...
   populateTS_2photon(Trr, Trv, Tvr, Tvv, sr, sv, Dtau, xe, xHII, TM, TR, nH, H, atomic, 
                      Dfplus, Dfplus_Ly, Alpha, DAlpha, Beta, fsR, meR, exclya, error, error_message);
   if (*error == 1) {
   ...
   solve_real_virt(xr, xv, Trr, Trv, Tvr, Tvv, sr, sv, error, error_message); 
...
double rec_dxHIIdlna(int model, double xe, double xHII, double nH, double H, double TM, double TR, 
  ...
  char sub_message[128];
  if (*error == 1) return 0.;
  if      (model == PEEBLES)  result = rec_TLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, 1.00, fsR, meR, ion, exclya);
  else if (model == RECFAST)  result = rec_TLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, 1.14, fsR, meR, ion, exclya);
  else if (model == EMLA2s2p) result = rec_HMLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, atomic, fsR, meR, ion, exclya, error, error_message);
    ...
    if (Pion < PION_MAX) { 
      result = rec_HMLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, atomic, fsR, meR, ion, exclya, error, error_message);
		...
		if (model == FULL) result = rec_HMLA_2photon_dxHIIdlna(xe, xHII, nH, H, TM, TR, atomic, 
			                             rad->Dfminus_hist, rad->Dfminus_Ly_hist, rad->Dfnu_hist, rad->z0,
                                         iz, z, fsR, meR, ion, exclya, error, error_message);
		else if (model == SWIFT) {
			if (TR/kBoltz < fit->swift_func[0][0]) {
				result = rec_HMLA_dxHIIdlna(xe, xHII, nH, H, TM, TR, atomic, fsR, meR, ion, exclya, error, error_message);
			}
