...
double xe_PostSahaHe(double nH0, double Tr0, double fHe, double H, double z){
   ...
   double Tr, nH, s, xeSaha, dxeSahadt, Dxe, DxedotDxe, ainv, Delta_xe;

   Tr = Tr0 * (ainv=1.+z); // current radiation temperature 
   nH = nH0*ainv*ainv*ainv;

   s = 2.414194e21*Tr*sqrt(Tr)*exp(-285325./Tr)/nH * 4.;
   xeSaha = rec_xesaha_HeII_III(nH0, Tr0, fHe, z);
   dxeSahadt = - xeSaha*(xeSaha-1.)/(2.*xeSaha +s-1.) * (285325./Tr - 1.5) * H;  // analytic derivative of xeSaha 
   
   Dxe = 0.01 * (1.+fHe-xeSaha);
   DxedotDxe = (rec_helium_dxHeIIdlna(xeSaha + Dxe, nH0, Tr0, fHe, H, z)
		- rec_helium_dxHeIIdlna(xeSaha - Dxe, nH0, Tr0, fHe, H, z))/2./Dxe;
...
double rec_helium_dxHeIIdlna(double xH1, double xHeII, double nH0, double Tr0, double fHe, double H, double z,
                             double fsR, double meR, int *error, char error_message[SIZE_ErrorM]) {
